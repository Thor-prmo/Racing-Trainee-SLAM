...
class Solution(Bot):
    ...
    def data_association(self, measurements, current_map):
        ...
        Steps:
          ...
          2. For each world-frame measurement find the nearest cone in
             *current_map* by Euclidean distance.

        Side-effects (used by the visualiser):
          self._global_meas  – (N, 2) world-frame measurement positions
          self._assoc        – (N,)   int indices into current_map

        Returns:
          np.ndarray of int, shape (N,), indices into current_map.
        """
        # ── Nothing to associate if there are no measurements ──────────
        ...
        # ── Step 1: local → world ──────────────────────────────────────
        # local_to_global handles the rotation + translation:
        #   world_pt = R(heading) @ local_pt + pos
        global_meas = local_to_global(measurements, self.pos, self.heading)

        # ── Step 2: nearest-neighbour matching ────────────────────────
        # cdist returns shape (N_meas, N_map); argmin over axis=1 gives
        # the index of the closest map cone for every measurement.
        D     = distance.cdist(global_meas, current_map)   # (N, M)
        assoc = np.argmin(D, axis=1).astype(int)           # (N,)
